...
def get_injected_layers(branch, target_layer, rank, clip_model):
    visual_width = clip_model.visual.transformer.width
    text_width = clip_model.transformer.width
    injected_layers = {}

    inject_c_fc_visual = get_inject_adapter(visual_width, rank, visual_width*4, clip_model)
    nn.init.constant_(inject_c_fc_visual.Linear1.weight, 1/visual_width)
    nn.init.constant_(inject_c_fc_visual.Linear2.weight, 1/(visual_width*rank*4))

    inject_c_proj_visual = get_inject_adapter(visual_width*4, rank, visual_width, clip_model)
    nn.init.constant_(inject_c_proj_visual.Linear1.weight, 1/(visual_width*rank*4))
    nn.init.constant_(inject_c_proj_visual.Linear2.weight, 1/visual_width)

    inject_c_fc_text = get_inject_adapter(text_width, rank, text_width*4, clip_model)
    nn.init.constant_(inject_c_fc_text.Linear1.weight, 1/text_width)
    nn.init.constant_(inject_c_fc_text.Linear2.weight, 1/(text_width*rank*4))

    inject_c_proj_text = get_inject_adapter(text_width*4, rank, text_width, clip_model)
    nn.init.constant_(inject_c_proj_text.Linear1.weight, 1/(text_width*rank*4))
    nn.init.constant_(inject_c_proj_text.Linear2.weight, 1/text_width)


    if branch == "visual":
        injected_layers["visual.mlp"] = {}   

        if target_layer == "c_fc":
            injected_layers["visual.mlp"]["c_fc"] = inject_c_fc_visual

        if target_layer == "c_proj":
            injected_layers["visual.mlp"]["c_proj"] = inject_c_proj_visual
    ...
    if branch == "transformer":
        injected_layers["text.mlp"] = {}
        
        if target_layer == "c_fc":
            injected_layers["text.mlp"]["c_fc"] = inject_c_fc_text

        if target_layer == "c_proj":
            injected_layers["text.mlp"]["c_proj"] = inject_c_proj_text
        ...
        for branch in ["both", "transformer", "visual"]:
            ...
            for target_layer in ["c_fc"]:
                ...
                for rank in [8]:
                    ...
                    injected_layers = get_injected_layers(branch, target_layer, rank, clip_model)
