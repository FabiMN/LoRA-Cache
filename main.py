        ...
        for branch in ["both", "transformer", "visual"]:
            ...
            for target_layer in ["c_fc"]:
                ...
                for rank in [8]:
                    ...
                    nn.init.zeros_(test_fc.B.weight)
                    ...
                    nn.init.zeros_(test_fc_text.B.weight)


                    if branch == "both":
                        injected_layers = {"visual.mlp": {f"{target_layer}": test_fc},
                                            "text.mlp": {f"{target_layer}": test_fc_text}}
                    
                    else:
                        target_branch = "visual"
                        if branch == "transformer":
                            target_branch = "text"
                            test_fc = test_fc_text

                        injected_layers = {f"{target_branch}.mlp": {f"{target_layer}": test_fc}}
